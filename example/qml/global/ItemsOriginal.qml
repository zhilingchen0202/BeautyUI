...
BObject{
    ...
    BPaneItemExpander{
        ...
        icon: BeautyIcons.CheckboxComposite
        BPaneItem{
            id: item_buttons
            count: 99
            infoBadge: BBadge{
        ...
        BPaneItem{
            ...
            disabled: true
            icon: BeautyIcons.Error
        }
    }

    BPaneItemExpander{
        title: qsTr("Form")
        icon: BeautyIcons.GridView
        BPaneItem {
            title: qsTr("TextBox")
            menuDelegate: paneItemMenu
    ...
    function getRecentlyAddedData(){
        ...
        for(var i=0;i<items.length;i++){
            var item = items[i]
            if(item instanceof BPaneItem && item.extra && item.extra.recentlyAdded){
                arr.push(item)
            }
        }
    ...
    function getRecentlyUpdatedData(){
        ...
        for(var i=0;i<items.length;i++){
            var item = items[i]
            if(item instanceof BPaneItem && item.extra && item.extra.recentlyUpdated){
                arr.push(item)
            }
        }
        return arr
    ...
    function getSearchData(){
        ...
        for(var i=0;i<items.length;i++){
            ...
            if(item instanceof BPaneItem){
                if (item.parent instanceof BPaneItemExpander)
                {
