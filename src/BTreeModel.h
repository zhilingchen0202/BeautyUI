...
public:
    explicit BTreeNode(QObject *parent = nullptr);
...
public:
    ...
    QVariantMap _data;
    QList<BTreeNode *> _children;
    BTreeNode *_parent = nullptr;
};

class BTreeModel : public QAbstractTableModel {
    Q_OBJECT
    Q_PROPERTY_AUTO(int, dataSourceSize)
    Q_PROPERTY_AUTO(QList<QVariantMap>, columnSource)
    QML_NAMED_ELEMENT(BTreeModel)
public:
    ...
    Q_INVOKABLE void insertRows(int row, const QList<BTreeNode *> &data);

    Q_INVOKABLE QObject *getRow(int row);

    Q_INVOKABLE void setRow(int row, QVariantMap data);

    Q_INVOKABLE void setData(QList<BTreeNode *> data);
    ...
    Q_INVOKABLE BTreeNode *getNode(int row);
...
private:
    QList<BTreeNode *> _rows;
    QList<BTreeNode *> _dataSource;
