...
BTheme::BTheme(QObject *parent) : QObject{parent} {
    _accentColor = BColors::getInstance()->Blue();
    _darkMode = BThemeType::DarkMode::Light;
    _nativeText = false;
    ...
    refreshColors();
    connect(this, &BTheme::darkModeChanged, this, [=] { Q_EMIT darkChanged(); });
    connect(this, &BTheme::darkChanged, this, [=] { refreshColors(); });
    connect(this, &BTheme::accentColorChanged, this, [=] { refreshColors(); });
    connect(&_watcher, &QFileSystemWatcher::fileChanged, this,
            [=](const QString &path) { Q_EMIT desktopImagePathChanged(); });
    connect(this, &BTheme::blurBehindWindowEnabledChanged, this,
            [=] { checkUpdateDesktopImage(); });
    startTimer(1000);
}

void BTheme::refreshColors() {
    auto isDark = dark();
...
bool BTheme::eventFilter(QObject *, QEvent *event) {
...
bool BTheme::dark() const {
    if (_darkMode == BThemeType::DarkMode::Dark) {
        return true;
    } else if (_darkMode == BThemeType::DarkMode::System) {
        return _systemDark;
    } else {
        return false;
    }
...
void BTheme::checkUpdateDesktopImage() {
    if (!_blurBehindWindowEnabled) {
        return;
    }
    QThreadPool::globalInstance()->start([=]() {
        _mutex.lock();
        auto path = BTools::getInstance()->getWallpaperFilePath();
...
void BTheme::timerEvent(QTimerEvent *event) {
    checkUpdateDesktopImage();
}
