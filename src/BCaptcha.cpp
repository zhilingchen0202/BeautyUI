...
BCaptcha::BCaptcha(QQuickItem *parent) : QQuickPaintedItem(parent) {
    _ignoreCase = false;
    QFont fontStyle;
#ifdef Q_OS_WIN
...
void BCaptcha::paint(QPainter *painter) {
    painter->save();
...
[[maybe_unused]] bool BCaptcha::verify(const QString &code) {
    if (_ignoreCase) {
