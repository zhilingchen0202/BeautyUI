...
/**
 * @brief The BTools class
 */
class BTools : public QObject {
    Q_OBJECT
    QML_NAMED_ELEMENT(BTools)
    QML_SINGLETON

private:
    explicit BTools(QObject *parent = nullptr);

public:
    SINGLETON(BTools)

    static BTools *create(QQmlEngine *, QJSEngine *) {
        return getInstance();
    }
