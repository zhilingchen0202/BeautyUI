...
BFrameless::BFrameless(QQuickItem *parent) : QQuickItem{parent} {
    _fixSize = false;
    _appbar = nullptr;
    ...
    _topmost = false;
    _disabled = false;
    _isWindows11OrGreater = BTools::getInstance()->isWindows11OrGreater();
}

BFrameless::~BFrameless() = default;

[[maybe_unused]] void BFrameless::onDestruction() {
    QGuiApplication::instance()->removeNativeEventFilter(this);
}

void BFrameless::componentComplete() {
    if (_disabled) {
        return;
    }
    int w = window()->width();
    ...
    connect(this, &BFrameless::topmostChanged, this, [this] {
        _setWindowTopmost(topmost());
    });
    _setWindowTopmost(topmost());
}

[[maybe_unused]] bool BFrameless::nativeEventFilter(const QByteArray &eventType, void *message, QT_NATIVE_EVENT_RESULT_TYPE *result) {
#ifdef Q_OS_WIN
...
bool BFrameless::_isMaximized() {
    return window()->visibility() == QWindow::Maximized;
}

bool BFrameless::_isFullScreen() {
    return window()->visibility() == QWindow::FullScreen;
}

void BFrameless::_showSystemMenu(QPoint point) {
#ifdef Q_OS_WIN
...
bool BFrameless::_hitMaximizeButton() {
...
void BFrameless::_setMaximizePressed(bool val) {
    if (_maximizeButton) {
        _maximizeButton->setProperty("down", val);
    }
}

void BFrameless::_setMaximizeHovered(bool val) {
    if (_maximizeButton) {
        _maximizeButton->setProperty("hover", val);
    }
}

void BFrameless::_updateCursor(int edges) {
    switch (edges) {
    case 0:
...
[[maybe_unused]] void BFrameless::showFullScreen() {
    window()->showFullScreen();
}

void BFrameless::showMaximized() {
#ifdef Q_OS_WIN
    HWND hwnd = reinterpret_cast<HWND>(window()->winId());
    ::ShowWindow(hwnd, 3);
#else
...
[[maybe_unused]] void BFrameless::showMinimized() {
#ifdef Q_OS_WIN
    HWND hwnd = reinterpret_cast<HWND>(window()->winId());
    ::ShowWindow(hwnd, 2);
#else
...
void BFrameless::showNormal() {
    window()->setVisibility(QQuickWindow::Windowed);
}

void BFrameless::setHitTestVisible(QQuickItem *val) {
    if (!_hitTestList.contains(val)) {
        _hitTestList.append(val);
    }
}

void BFrameless::_setWindowTopmost(bool topmost) {
#ifdef Q_OS_WIN
    HWND hwnd = reinterpret_cast<HWND>(window()->winId());
    if (topmost) {
...
bool BFrameless::eventFilter(QObject *obj, QEvent *ev) {
#ifndef Q_OS_WIN
    switch (ev->type()) {
