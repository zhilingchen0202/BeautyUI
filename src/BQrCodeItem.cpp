...
BQrCodeItem::BQrCodeItem(QQuickItem *parent) : QQuickPaintedItem(parent) {
    _color = QColor(0, 0, 0, 255);
    _bgColor = QColor(255, 255, 255, 255);
    _size = 100;
    setWidth(_size);
    setHeight(_size);
    connect(this, &BQrCodeItem::textChanged, this, [=] { update(); });
    connect(this, &BQrCodeItem::colorChanged, this, [=] { update(); });
    connect(this, &BQrCodeItem::bgColorChanged, this, [=] { update(); });
    connect(this, &BQrCodeItem::sizeChanged, this, [=] {
        setWidth(_size);
        setHeight(_size);
        update();
    });
...
void BQrCodeItem::paint(QPainter *painter) {
    if (_text.isEmpty()) {
        return;
    }
